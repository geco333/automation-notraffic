...
from tests.pom import LoginPage
...
def _xdist_worker_id() -> str | None:
    """Return current xdist worker id (e.g. gw0) or None."""
    return os.environ.get("PYTEST_XDIST_WORKER")


def _is_primary_worker() -> bool:
    """True for non-xdist runs and primary xdist worker."""
    worker_id = _xdist_worker_id()
    return worker_id is None or worker_id == "gw0"


def _wait_for_auth_state(timeout_seconds: int = AUTH_STATE_WAIT_SECONDS) -> bool:
    """Wait for auth state file created by the primary worker."""
...
def perform_authentication_first(browser):
    ...
    # In parallel runs, only primary worker creates auth state.
    if not _is_primary_worker():
        if not _wait_for_auth_state():
            raise RuntimeError(
                f"Timed out waiting for auth state file: {AUTH_STATE_FILE}"
            )
        logger.info("Using authentication state generated by primary worker")
        return
...
def page(context: BrowserContext):
    ...
    This fixture creates a page within the context with auth state applied.
    It restores sessionStorage values that Playwright storage_state does not restore.
